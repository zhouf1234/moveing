...
from . models import Area,Cata,NormalUser,Resource
...
def MoveList(request):
    ...
    return render(request, 'moves.html',context=context)

#查看资源详情
def detailMove(request):
    m_id = request.POST.get('m_id',None)
    res = Resource.objects.get(id=m_id)
    context = {
        'res': res,
    }
    return render(request,'detail_res.html',context=context)
